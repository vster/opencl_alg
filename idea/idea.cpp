...
* IDEA Decryption                                *
*************************************************/
void IDEA::decrypt(const byte in[BLOCKSIZE], byte out[BLOCKSIZE]) const
   {
   u16bit X1 = make_u16bit(in[0], in[1]), X2 = make_u16bit(in[2], in[3]),
          X3 = make_u16bit(in[4], in[5]), X4 = make_u16bit(in[6], in[7]);
   for(u32bit j = 0; j != 8; j++)
      {
      mul(X1, DK[6*j+0]); X2 += DK[6*j+1]; X3 += DK[6*j+2];
      mul(X4, DK[6*j+3]); u16bit T1 = X3; X3 ^= X1; mul(X3, DK[6*j+4]);
      u16bit T0 = X2; X2 = (u16bit)((X2 ^ X4) + X3); mul(X2, DK[6*j+5]);
...
*************************************************/
void IDEA::mul(u16bit& a, u16bit b)
   {
   if(a && b)
      {
...
* IDEA Key Schedule                              *
*************************************************/
void IDEA::set_key(const byte key[], u32bit length) throw(InvalidKeyLength)
   {
