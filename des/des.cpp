...
* DES Encryption                                 *
*************************************************/
void DES::encrypt(const byte in[BLOCKSIZE], byte out[BLOCKSIZE]) const
   {
...
* DES Decryption                                 *
*************************************************/
void DES::decrypt(const byte in[BLOCKSIZE], byte out[BLOCKSIZE]) const
   {
   u32bit left  = make_u32bit(in[0], in[1], in[2], in[3]),
          right = make_u32bit(in[4], in[5], in[6], in[7]);
   IP(left, right);
...
void DES::IP(u32bit& L, u32bit& R)
   ...
   L = (T >> 32) & 0xFFFFFFFF; R = T & 0xFFFFFFFF;
   }
...
* DES Key Schedule                               *
*************************************************/
void DES::set_key(const byte key[], u32bit length) throw(InvalidKeyLength)
   {
...
*************************************************/
void TripleDES::encrypt(const byte in[BLOCKSIZE], byte out[BLOCKSIZE]) const
   {
   u32bit left  = make_u32bit(in[0], in[1], in[2], in[3]),
          right = make_u32bit(in[4], in[5], in[6], in[7]);
   DES::IP(left, right);
...
*************************************************/
void TripleDES::decrypt(const byte in[BLOCKSIZE], byte out[BLOCKSIZE]) const
   {
   u32bit left  = make_u32bit(in[0], in[1], in[2], in[3]),
          right = make_u32bit(in[4], in[5], in[6], in[7]);
   DES::IP(left, right);
...
*************************************************/
void TripleDES::set_key(const byte key[], u32bit length)
   throw(InvalidKeyLength)
   {
   if(!valid_keylength(length))
      throw InvalidKeyLength(name(), length);
   des1.set_key(key, BLOCKSIZE);
   des2.set_key(key + BLOCKSIZE, BLOCKSIZE);
   des3.set_key(key + 2 * BLOCKSIZE, BLOCKSIZE);
   }
